...
Component: j-Columns
Version: 1
Updated: 2023-01-23T11:07:18.000Z
Author: Peter Širka
...
Component: j-iFramePreview
Version: 1
Updated: 2023-05-24T08:08:00.000Z
Author: Peter Širka
...
Version: 1
Updated: 2023-08-28T15:38:00.000Z
Author: Peter Širka


Component: j-MarkdownPreview
Version: 1
Updated: 2023-01-31T15:43:00.000Z
Author: Peter Širka
...
.ui-columns > section{float:left;border-left:1px solid #E0E0E0}.ui-columns > section:first-child{border-left:0}.ui-columns-noborder > section{border-left:0 !important}.ui-dark .ui-columns > section{border-left-color:#404040}
...
.ui-iframepreview{line-height:0}.ui-iframepreview iframe{width:100%;height:100%;padding:0;margin:0;display:block}
...
COMPONENT('formdata','method:POST;target:_blank;type:application/x-www-form-urlencoded',function(self,config){var form;self.singleton();self.readonly();self.make=function(){self.aclass('hidden');form=document.createElement('FORM');self.dom.appendChild(form)};self.send=function(opt){var data=opt.data||opt.value;while(form.children.length)form.removeChild(form.children[0]);form.setAttribute('method',opt.method||config.method);form.setAttribute('type',opt.type||config.type);form.setAttribute('target',opt.target||config.target);form.setAttribute('action',opt.url||opt.action);for(var key in data){var input=document.createElement('INPUT');input.setAttribute('name',key);input.setAttribute('type','hidden');input.value=data[key];form.appendChild(input)}form.submit()}});
...
COMPONENT('iframepreview',function(self,config,cls){var iframe,ready=false,writetimeout,container;self.readonly();self.nocompile&&self.nocompile();self.make=function(){self.aclass(cls+' hidden');self.html('<div><iframe src="about:blank" frameborder="0" scrolling="no" loading="lazy"></iframe></div>');iframe=self.find('iframe');container=self.find('div');var isready=()=>ready=true;var timeout=null;iframe.on('load',function(){timeout&&clearTimeout(timeout);timeout=setTimeout(isready,150)});timeout=setTimeout(isready,1000)};self.destroy=function(){iframe=null};self.write=function(content){writetimeout&&clearTimeout(writetimeout);writetimeout=null;if(!ready){writetimeout=setTimeout(self.write,200,content);return}var win=iframe&&iframe[0]?iframe[0].contentWindow:null;if(win){var is=false,offset='<div id="IFPOFFSET"></div>';if(content.indexOf('<body')===-1)content='<!DOCTYPE html><html><head><meta charset="utf-8" /></head><body style="font-family:Arial;font-size:14px;margin:0;padding:0">'+content+'</body></html>';content=content.replace(/<\/body>/i,function(){is=true;return offset+'</body>'});if(!is)content+=offset;var doc=win.document;doc.open();doc.write(content);doc.close();self.resize();self.rclass('hidden');setTimeout(self.resize,500);setTimeout(self.resize,1000);setTimeout(self.resize,2000);setTimeout(self.resize,3000)}};self.resize=function(){var win=iframe&&iframe[0]?iframe[0].contentWindow:null;if(win){var el=$(win.document.getElementById('IFPOFFSET'));if(el.length){var offset=el.offset();if(offset){container.css('height',offset.top);if(offset.top==0)setTimeout(self.resize,1000)}}}};self.setter=function(value){self.aclass('hidden');if(value)self.write(value);else iframe.attr('src','about:blank')}});
